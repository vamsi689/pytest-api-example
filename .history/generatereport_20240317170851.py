...
def set_npm_path():
    # Path to the directory containing the npm executable
    npm_path = r'C:\Program Files\nodejs'  # Adjust this path as per your npm installation

    # Get the existing PATH environment variable
    path = os.environ.get('PATH', '')

    # Add the npm directory to the PATH if it's not already there
    if npm_path not in path:
        os.environ['PATH'] = f"{path};{npm_path}"
